...
public class OnConfirmBuilder {
    ...
    private void buildOrder(ConfirmMessage message, OnConfirmMessage response) {
        ...
        try {
            List<com.ms.bpp.entity.Order> orderList=orderRepo.findAll();
//            if(orderList.stream().filter(Objects::nonNull).anyMatch(order -> order.getSessionAttendeeDetails().getEmail().equalsIgnoreCase(fulfillment.getCustomer().getPerson().getMailid()) && order.getSkillRegisterFor().equalsIgnoreCase(provider.getSkill().get(0).getName()))){
//
//            }else {
